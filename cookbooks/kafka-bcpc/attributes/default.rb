...
default[:kafka][:base_url] = Bcpc::OSHelper.get_binary_server_url(node) + "kafka"
default[:kafka][:broker][:host_name] = Bcpc::OSHelper.float_host(node,node[:fqdn])
default[:kafka][:broker][:advertised_host_name] = Bcpc::OSHelper.float_host(node,node[:fqdn])
