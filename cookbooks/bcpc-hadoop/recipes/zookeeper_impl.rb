...
ruby_block "Compare_zookeeper_server_start_shell_script" do
  block do
    ...
    new_checksum=Digest::MD5.hexdigest(File.read("/usr/hdp/current/zookeeper-server/bin/zkServer.sh"))
...
link "/usr/bin/zookeeper-server-initialize" do
  to "/usr/hdp/current/zookeeper-server/bin/zookeeper-server-initialize"
end
...
template "/usr/hdp/current/zookeeper-server/bin/zkServer.sh" do
...
service "zookeeper-server" do
  ...
  subscribes :restart, "template[/usr/hdp/current/zookeeper-server/bin/zkServer.sh]", :delayed
