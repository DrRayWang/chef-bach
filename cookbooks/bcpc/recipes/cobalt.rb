...
if not node["bcpc"]["vms_key"].nil?
    ...
    bash "create-vms-disk-rados-pool" do
        user "root"
        optimal = Bcpc::Helper.power_of_2(Bcpc::OSHelper.get_ceph_osd_nodes(node, method( :search )).length*node[:bcpc][:ceph][:pgs_per_node]/node[:bcpc][:ceph][:vms_disk][:replicas]*node[:bcpc][:ceph][:vms_disk][:portion]/100)
        code <<-EOH
    ...
    bash "create-vms-mem-rados-pool" do
        user "root"
        optimal = Bcpc::Helper.power_of_2(get_cepg_osd_nodes.length*node[:bcpc][:ceph][:pgs_per_node]/node[:bcpc][:ceph][:vms_mem][:replicas]*node[:bcpc][:ceph][:vms_mem][:portion]/100)
        code <<-EOH
    ...
    bash "set-vms-mem-rados-pool-pgs" do
        user "root"
        optimal = Bcpc::Helper.power_of_2(Bcpc::OSHelper.get_ceph_osd_nodes(node, method( :search )).length*node[:bcpc][:ceph][:pgs_per_node]/node[:bcpc][:ceph][:vms_mem][:replicas]*node[:bcpc][:ceph][:vms_mem][:portion]/100)
